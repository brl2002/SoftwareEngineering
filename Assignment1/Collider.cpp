...
bool Collider::IsMoveColliding(GameObject* other, float dt)
{
	double x = m_gameObject->GetPosition().x - m_gameObject->getCollider()->getWidth() / 2 + m_gameObject->GetVelocityX() * dt;
	double y = m_gameObject->GetPosition().y + m_gameObject->getCollider()->getHeight() / 2 + m_gameObject->GetVelocityY() * dt;
	double width = m_gameObject->getCollider()->getWidth();
	double height = m_gameObject->getCollider()->getHeight();
...
void Collider::SetFlag(ColliderFlag flag)
{
	m_flag = flag;
}

ColliderFlag Collider::GetFlag()
{
	return m_flag;
}